...
def test_repair_rebuilds_stale_btree_indexes(tmp_path):
    ...
    assert reason is not None
    # SQLite versions describe this same on-disk defect as either an entry
    # count mismatch or missing indexed rows. Require the affected index and
    # corruption class, then verify the real REINDEX repair below.
