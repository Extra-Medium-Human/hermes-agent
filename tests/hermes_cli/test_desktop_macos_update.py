...
int main(int n,char**v){
 if(n>2&&!strcmp(v[1],"map")){
  int fd=open(v[2],O_RDONLY);struct stat st;
  if(fd<0||fstat(fd,&st))return 3;
  void *p=mmap(NULL,st.st_size,PROT_READ,MAP_PRIVATE,fd,0);close(fd);
  if(p==MAP_FAILED)return 4;
  puts("ready");fflush(stdout);char c;read(STDIN_FILENO,&c,1);
  munmap(p,st.st_size);return 0;
 }
...
def test_native_observation_finds_exact_bundle_readers(bundles, native_program, reader):
    root, _, target = bundles
    update = updater()
    ...
    if reader == "mapped-only":
        process = subprocess.Popen([str(native_program), "map",
